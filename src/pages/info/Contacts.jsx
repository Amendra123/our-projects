...
const Contacts = (props) => {
  ...
  const handleItemChange = (event, id) => {
    ...
    if (isJobWorkCategory) {
      handleCellChange(id, "rate", 0);
      if (newItem === "1R0010001") {handleCellChange(id, "rate", 0);
...
const recalculateDiscountAmount = (newRate, newDiscountPercentage, id) => {
  ...
  const finalDiscountAmount = quantity * newRate - newDiscountAmount.toFixed(2);
  setRows((prevRows) => {
  ...
  const handleDeleteRow = (id) => {
    setRows((prevRows) => {
      ...
      const updatedTotalDiscountAmount = calculateTotalDiscountAmount(updatedRows);
      setTotalDiscountAmount(updatedTotalDiscountAmount.toFixed(2));
      const updatedTotalAmountWith18Percent = updatedTotalDiscountAmount * 1.18;
      setTotalAmountWith18Percent(updatedTotalAmountWith18Percent.toFixed(2));
